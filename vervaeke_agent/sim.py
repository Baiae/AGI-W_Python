"""Simulation logic for the Vervaeke Agent."""
...
class BeliefEngine(BaseModel):
    ...
    def init_priors_if_empty(self) -> None:
        ...
        self.beliefs["SAFE_MARKER_TOWARD"] = Belief(
            description="Following SAFE markers yields good outcomes"
        )
        self.beliefs["DANGER_MARKER_AWAY"] = Belief(
            description="Avoiding DANGER markers preserves Integrity"
        )
    ...
    def get_confidence(self, key: str) -> float:
        return self.beliefs[key].confidence if key in self.beliefs else 0.5
...
class HeadlessEnv(BaseModel):
    ...
    def add_agent(self, agent_id: str, x: int | None = None, y: int | None = None) -> None:
        ...
        if x is None or y is None:
            ...
            if not (0 <= x < self.width and 0 <= y < self.height):
                raise ValueError(
                    f"Spawn out of bounds: ({x},{y}) for grid {self.width}x{self.height}"
                )
            if not self._is_spawnable(x, y):
                raise ValueError(
                    f"Spawn cell not EMPTY or occupied at ({x},{y}) (cell={self.grid[y][x]})"
                )
    ...
    def get_env_data(self, agent_id: str) -> EnvData:
        ...
        return EnvData(
            ...
            safe_marker_dist=safe_dist,
            safe_dir=safe_dir,
            danger_marker_dist=danger_dist,
            danger_dir=danger_dir,
            home_marker_dist=home_dist,
            home_dir=home_dir,
        )
    ...
    *,
    ...
    for _ in range(total_steps):
        ...
        if (
            not quiet
            and report_every
            and (state.tick % report_every == 0 or state.tick == state.betrayal_step)
        ):
            print(f"\n=== Tick {state.tick} ===")
            for aid, agent in state.agents.items():
